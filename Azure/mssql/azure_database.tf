...
resource "random_password" "root-auth" {
  length      = 20
  min_lower   = 5
  min_upper   = 5
  min_numeric = 5
...
resource "azurerm_sql_server" "demo" {
  ...
  administrator_login_password = random_password.root-auth.result
}
...
resource "azurerm_sql_firewall_rule" "demo-allow-ssh-instance" {
